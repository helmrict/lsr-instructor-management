...
class LSIM_Instructor_Fields {
        ...
        jQuery(document).ready(function($) {
            ...
            $('input[type="date"]').on('change', function() {
                ...
                if (dates.length > 0) {
                    ...
                    // Add certification period years
                    const expiration = new Date(mostRecent);
                    expiration.setFullYear(expiration.getFullYear() + <?php echo $certification_period; ?>);
                    
                    // Update expiration info
                    section.find('.expiration-info').html(
                        'Expires: ' + expiration.toLocaleDateString('en-US', {
                            year: 'numeric',
                            month: 'long',
                            day: 'numeric'
                        })
    ...
    private function get_certification_expiration($post_id, $type) {
        ...
        return date('Y-m-d', strtotime($most_recent . " +{$certification_period} years"));
    }
